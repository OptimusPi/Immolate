...
long filter(instance *inst)
{
    ...
    for (int ante = 1; ante <= 20; ante++)
    {
        ...
        state.score += checkTag(inst, ante, &state);
        int cardSearch = ante > 2 ? ante > 4 ? ante > 6 ? ante > 8 ? ante > 10 ? 200 : 96 : 48 : 24 : 12 : 6;
        ...
        if (ante > 4) {
            if (state.perkeo == false) return 0;
            if (state.blueprints + state.brainstorms < 1) return 0;
            //if (state.hasSpaceman == false && state.hasBurnt == false) return 0;
            
            //if (state.hasLuckyCat == false) return 0; 
            
            //if (state.chads < 1 && state.socks < 1) return 0;
            //if (state.dna == 0) return 0;
        } else if (ante > 10) {
            if (state.dice < 2) return 0;
            if (state.hasSpaceman == false) return 0;
            if (state.hasLuckyCat == false) return 0;
            if (state.observatory == false) return 0;
            if (state.blueprints + state.brainstorms < 4) return 0;
            if (state.dna == 0) return 0;
        }
    }
    ...
    //if (state.perkeo + state.canio == 0) return 0;

    //if (state.pareidolia == 0) return 0;
