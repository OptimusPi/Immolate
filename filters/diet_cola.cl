...
void checkJoker(jokerdata joker, game_state* state) {
    if (state->ante == 1 && joker.joker == Diet_Cola) {
        state->dietColaAnte1 = true;
    }
}
...
long filter(instance* inst) {
    ...
    return state.dietColaAnte1 && state.immolateAnte1 ? state.ankhCount : 0;
}
