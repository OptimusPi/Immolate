...
import time
import pandas as pd
# Import pandastable for better data display with pandas integration
...
def load_configuration_from_path(file_path):
    ...
    try:
        ...
        config_name = config.get("name", os.path.basename(file_path).replace('.ouija.json', ''))
...
def refresh_results_from_duckdb():
    ...
    try:
        if duckdb_conn:
            ...
            # Query all results with pandas, sorted by Score descending
            df = duckdb_conn.execute('SELECT * FROM results ORDER BY "Score" DESC').fetch_df()
            
            # Update the pandastable with new data
            if 'pt' in globals() and pt:
...
def run_ouija_cmd():
    ...
    if needs_list or wants_list:
        ...
        if config_loaded_from_file and not config_modified:
            ...
            temp_config_path = f"ouija_configs/{config_name}_{int(time.time())}.ouija.json"
    ...
    try:
        ...
        def read_output():
            ...
            try:
                ...
                while True:
                    ...
                    if line.startswith("|"):
                        ...
                        if duckdb_conn:
                            ...
                            # Add to result_rows for immediate display
                ...
                # If we have collected results, update the pandas table
                if result_rows and header_columns:
                    df = pd.DataFrame(result_rows, columns=header_columns)
                    pt.updateModel(TableModel(df))
                    pt.redraw()
                ...
                output_text.insert(tk.END, f"Error reading process output: {str(e)}\n")
                output_text.see(tk.END)
        ...
        messagebox.showerror("Error", "Ouija.exe not found in the current directory.")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to run Ouija.exe: {e}")
...
def export_configuration():
    ...
    # Create configuration object matching perkeo_finder.ouija.json format
    config = {
        "name": config_name,
        "description": f"Filter configuration created by Ouija GUI on {datetime.now().strftime('%Y-%m-%d')}",
        "author": "Ouija GUI User",
        "filter_config": {
            "numNeeds": len(needs_list),
            "numWants": len(wants_list),
    ...
    suggested_filename = config_name.lower().replace(" ", "_") + ".ouija.json"
    
    # Make sure the ouija_configs directory exists
    os.makedirs("ouija_configs", exist_ok=True)
    
    file_path = filedialog.asksaveasfilename(
        initialdir="ouija_configs",
...
print("Starting Ouija GUI...")

# Create the main window
root = tk.Tk()
root.title("Ouija - Balatro Seed Finder by pifreak")
# Set default window size to 720p
root.geometry("1200x720")

# Set the background color of the main window to #394D53 (grey)
root.configure(bg="#394D53")# Options: "light" or "dark"
...
run_button = tk.Button(run_settings_frame, text="Let Jimbo Cook!", command=run_ouija_cmd, bg=BLUE, fg="white", font=("m6x11", 18, "bold"), height=2, width=20)
...
# Create an initial empty DataFrame for the table
initial_df = pd.DataFrame({'Seed': [], 'Score': []})

# Create the pandas table with Excel-like features
pt = Table(results_frame, dataframe=initial_df, showtoolbar=True, showstatusbar=True)
pt.show()

# Configure table appearance and behavior
pt.autoResizeColumns()
pt.rowheader.bgcolor = '#394D53'  # Match app theme
pt.colheader.bgcolor = BLUE       # Match app theme
pt.cellbackgr = '#2E3B42'         # Slightly darker than main bg for contrast
pt.textcolor = 'white'            # White text for better readability
pt.fontsize = 12                  # Larger font for readability
pt.thefont = ('m6x11', 12)        # Match app font
pt.rowselectedcolor = RED         # Highlight selected rows with app theme color
pt.colselectedcolor = '#BBDEFB'   # Light blue for column selection
