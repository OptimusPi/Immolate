...
class AdvancedSettingsDialog(tk.Toplevel):
    """Dialog window for advanced settings (thread groups, GPU batch, cutoff)."""

    def __init__(self, parent, thread_groups, gpu_batch, cutoff, controller=None):
        super().__init__(parent)
        self.title("Advanced Settings")
        self.geometry("350x250")  # Increased height for devil button
        self.resizable(False, False)
        self.configure(bg=BACKGROUND)
        self.result = None
        self.controller = controller
        self.prank_button_clicked = False
        ...
        tk.Entry(
            content_frame, textvariable=self.thread_groups_var, font=("m6x11", 12)
        ...
        tk.Entry(
            content_frame, textvariable=self.gpu_batch_var, font=("m6x11", 12)
        ).grid(row=row, column=1, sticky="ew", pady=5)
        row += 1

        # Cutoff
        tk.Label(
            content_frame, text="Cutoff:", bg=BACKGROUND, fg="white", font=("m6x11", 12)
        ).grid(row=row, column=0, sticky="e", padx=(0, 10), pady=5)
        self.cutoff_var = tk.StringVar(value=cutoff)
        tk.Entry(content_frame, textvariable=self.cutoff_var, font=("m6x11", 12)).grid(
            row=row, column=1, sticky="ew", pady=5
        )
        row += 1

        # Devil Button for Prank Seeds
        prank_frame = tk.Frame(content_frame, bg=BACKGROUND)
        prank_frame.grid(row=row, column=0, columnspan=2, pady=(15, 5), sticky="ew")

        self.prank_button = tk.Button(
            prank_frame,
            text="Do Not Push This Button! 😈",
            command=self.on_prank_button,
            bg="#8B0000",  # Dark red
            fg="white",
            font=("m6x11", 12),
            relief="raised",
        ...
        self.prank_button.pack(fill="x", padx=10)
        row += 1
    ...
    def on_prank_button(self):
        """Handle the prank seed button click"""
        if self.prank_button_clicked:
            return  # Prevent multiple clicks

        self.prank_button_clicked = True

        # Change button appearance
        self.prank_button.config(
            text="🙈🙉🙊", bg="#666666", state="disabled"  # Gray out
        )

        # Close this dialog first
        self.destroy()

        # Run prank seed search through controller
        if self.controller:
            self.controller.run_prank_seed_search()

    @staticmethod
    def show_dialog(parent, thread_groups, gpu_batch, cutoff, controller=None):
        dialog = AdvancedSettingsDialog(
            parent, thread_groups, gpu_batch, cutoff, controller
        )
