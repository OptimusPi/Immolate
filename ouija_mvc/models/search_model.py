...
class SearchModel:
    ...
    def build_command(
        self, config_path, starting_seed, thread_groups, number_of_seeds, template=None
    ):
        ...
        if isinstance(starting_seed, str) and (
            starting_seed.lower() == "random" or not starting_seed.strip()
        ):
            command_parts.extend(["-s", "random"])
        else:
            command_parts.extend(
                ["-s", str(starting_seed).upper()]
            )  # Ensure starting_seed is string

        # Add thread groups
        thread_groups_value = self.THREAD_GROUP_MAP.get(
            str(thread_groups), "32"
        ...
        if db_model and db_model.conn and db_model.table_exists():
            ...
            try:
                existing_df = db_model.query_results(
                    limit=1
                )  # Changed from get_dataframe(limit=1)
    ...
    def stop_all_searches(self):
        ...
        try:
            if os.name == "nt":  # Windows
                ...
                subprocess.call(
                    ["taskkill", "/F", "/IM", "Ouija.exe"], stderr=subprocess.DEVNULL
                )
            else:  # Unix/Linux/Mac
