...
class FunSearchController:
    ...
    def run_fun_seed_search(self, category):
        ...
        try:
            fun_words = {
                ...
                "COOL": ["LUCK", "MINT", "FIRE", "DOPE", "SICK", "EPIC", "RAGE", "WILD", "YEAH", "BOSS", "HERO", "STAR", "GOLD", "RICH", "DAMN", "MEGA", "HUGE", "ROCK", "KING"]
            }
            ...
            if self.search_model.has_active_searches() or self.fun_search_active:
                if self.current_view:
                    self.current_view.write_to_console("⚠️ A search is already running!\n")
            ...
            return Result.success(f"{category} fun search started") if result else Result.error("Failed to start fun search")
    ...
    def handle_search_completed(self):
        ...
        try:
            ...
            if self.current_view:
                self.current_view.write_to_console(f"⚠️ IndexError in fun search completion: {e}\n")
            print(f"IndexError in handle_search_completed: {e}")
